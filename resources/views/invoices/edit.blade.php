                    ...
                    <form action="{{ route('invoices.update', $invoice->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="mb-4">
                                ...
                                <input type="number" name="number" id="number" value="{{ $invoice->number }}"
                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                       required>
                            </div>
                            <div class="mb-4">
                                <label for="date" class="block text-gray-700 text-sm font-bold mb-2">Date:</label>
                                <input type="date" name="date" id="date" value="{{ $invoice->date->format('Y-m-d') }}"
                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                       required>
                            </div>
                            <div class="mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="pay_mode_id" id="pay_mode_id"
                                    ...
                                    <option value="{{ $pay_mode->id }}" {{ $invoice->pay_mode_id == $pay_mode->id ? 'selected' : '' }}>
                                        {{ $pay_mode->name }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div id="items-container">
                                ...
                                <div class="item-row grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="items[{{ $index }}][quantity]" min="1" value="{{ $detail->quantity }}"
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="items[{{ $index }}][price]" min="0" step="0.01" value="{{ $detail->price }}"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('add-item').addEventListener('click', function() {
                ...
                newRow.querySelectorAll('select, input').forEach(el => {
                    const name = el.getAttribute('name').replace(/\[\d+\]/, `[${itemCount}]`);
                    el.setAttribute('name', name);
                    if (!el.classList.contains('product-select') && !el.classList.contains('price-input')) {
                        el.value = '';
                    }
                });
